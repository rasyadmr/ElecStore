...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg my-20">
    ...
    <h1 class="font-bold text-3xl text-end m-4 text-black dark:text-white">Total: Rp @php
        echo $total;
    @endphp</h1>
            ...
            <div class="p-auto space-y-4 md:space-y-6 sm:p-8">
                ...
                <form class="space-y-4 md:space-y-6" action="{{ route('checkout.paid') }}" method="POST">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" placeholder="Full Name" class="bg-gray-50 border border-gray-300 text-gray-500 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="username" value="{{ old('name', auth()->user()->name) }}" disabled required>
                    </div>
                    <div>
                        <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                        <input type="email" name="email" id="email" placeholder="user@email.com" class="bg-gray-50 border border-gray-300 text-gray-500 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ old('email', auth()->user()->email) }}" disabled required>
                    </div>
                    <div>
                    ...
                    <div>
                        <label for="payment_method" class="block mb-2 text-sm font-medium 
                        @error('payment_method')
                            text-red-700 dark:text-red-500
                        @else
                            text-gray-900 dark:text-white
                        @enderror">Select payment method</label>
